...
std::vector<CrossFieldIntegerProgram::Edge> MIQQuad::buildInternalEdges() const
{
    std::vector<CrossFieldIntegerProgram::Edge> internal;
    internal.reserve(edgeList.size());
    for (const auto& e : edgeList) {
        if (e.f1 < 0 || e.f2 < 0) continue;
        CrossFieldIntegerProgram::Edge ie;
        ie.f1 = e.f1;
        ie.f2 = e.f2;
        ie.idx = e.idx;
...
{
    auto internal = buildInternalEdges();
    crossFieldIP_ = std::make_unique<CrossFieldIntegerProgram>(nF, std::move(internal));
    crossFieldIP_->setJumpBounds(jumpLo_, jumpHi_);
    crossFieldIP_->setAlternatingIterations(crossIters_);
    crossFieldIP_->setRefinePasses(jumpRefinePasses_);
    return crossFieldIP_->solve(theta, jump);
}
