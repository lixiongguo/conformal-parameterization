...
QuadCover::QuadCover(Mesh& mesh0)
    : Parameterization(mesh0),
      hasExternalFaceDirs(false),
      smoothIters(3),
      requirePureQuads(true) {}

void QuadCover::setFaceDirections(const MatrixXd& dirs) {
...
void QuadCover::estimatePrincipalCurvature() {
    ...
    for (int fi = 0; fi < nFaces; ++fi) {
        ...
        if (hasExternalFaceDirs && externalFaceDirs.rows() == nFaces) {
            ...
            if (dlen > 1e-12) faceDirs.row(fi) = dir / dlen;
            else {
                Vector3d t1, t2;
    ...
    if (!(hasExternalFaceDirs && externalFaceDirs.rows() == nFaces)) {
        PrincipalCurvatureField::estimateFromWeingarten(
            vertPos, faces, vertexNormals, faceNormals, faceDirs);
    }
}
...
void QuadCover::syncFaceDirsFromTheta() {
    for (int fi = 0; fi < nFaces; ++fi) {
        ...
        faceDirs.row(fi) = (c * t1 + s * t2).normalized();
    }
}
...
void QuadCover::computeMatching() {
    ...
    for (int eIdx = 0; eIdx < nEdges; ++eIdx) {
        ...
        Vector2d d1_2d(faceDirs.row(e.f1).dot(t11), faceDirs.row(e.f1).dot(t12));
        Vector2d d2_2d(faceDirs.row(e.f2).dot(t21), faceDirs.row(e.f2).dot(t12));
