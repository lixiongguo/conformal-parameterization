...
BFF::BFF(Mesh& mesh0) : CutSeamParameterization(mesh0), mode(CURVATURE), scale(1.0) {}
...
void BFF::computeBoundaryCurvature() {
    ...
    EdgeCIter startEdge = cutMesh_.edges.end();
    for (EdgeCIter e = cutMesh_.edges.begin(); e != cutMesh_.edges.end(); e++) {
        if (e->isBoundary()) { startEdge = e; break; }
    }
    if (startEdge == cutMesh_.edges.end()) return;
