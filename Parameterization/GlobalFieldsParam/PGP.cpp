...
{
    faceDirs_.resize(nF, 3);

    if (hasExternalField_ && faceDirs_.rows() == nF) {
        for (int f = 0; f < nF; ++f) {
            Vector3d n = faceNormals.row(f);
            Vector3d d = faceDirs_.row(f);
            d -= d.dot(n) * n;
            double len = d.norm();
            faceDirs_.row(f) = (len > 1e-12) ? (d / len) : Vector3d::UnitX();
        }
    } else {
        ...
        for (int f = 0; f < nF; ++f) {
            ...
            faceDirs_.row(f) = (len > 1e-12) ? (best / len) : Vector3d::UnitX();
        }
    }
